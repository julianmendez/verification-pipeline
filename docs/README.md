...
The **Verification Pipeline** is an instantiation of the [Tiles][tiles] framework that operationalizes action language
semantics through executable verification pipelines.
Its classes are written in [Soda][soda] and grouped in packages translated to [Scala][scala].
It process states, actions, transitions, and rules.


## Requirements

This project requires [Java][java] 17 or higher and [sbt][sbt] to build th project.
It is also possible to compile the project by using the [sbt script][sbt-script] provided by the
project [sbt-extras][sbt-extras].


## Use

This project can be compiled by executing [makeall.sh][makeall].
The script can be run on Windows with [Git Bash][git-scm].

The script compiles the project, creates the executable binary `verify`, and creates the directory `target/benchmarks`
with an example to run the program.
The created directory includes the script [run_benchmarks.sh][runbenchmarks], which can be configured to execute the
binary on trajectories generated by cyclically replicating a pattern.
Note that recompiling the project will delete the content of that directory to reset the conditions for new benchmarks.

More details on the YAML input file can be found at the [input file details][about-input-file].
...
[makeall]: https://github.com/julianmendez/verification-pipeline/blob/master/makeall.sh

[release-notes]: https://julianmendez.github.io/tiles/RELEASE-NOTES.html

[runbenchmarks]: https://github.com/julianmendez/verification-pipeline/blob/master/core/src/main/bash/run_benchmarks.sh

[sbt]: https://www.scala-sbt.org/

[sbt-extras]: https://github.com/dwijnand/sbt-extras

[sbt-script]: https://github.com/dwijnand/sbt-extras/blob/master/sbt
